...
def make_objective(X_train, y_train, X_val, y_val, class_weight):
    """
...
def train_lightgbm():
    print("\n" + "ðŸ”¶ " * 25)
    print("  LightGBM Training (No Leakage - Validation Tuning)")
    print("ðŸ”¶ " * 25)
    ...
    print(f"\n  class_weight = {class_weight}")
    print(f"  (Fraud cases weighted {class_weight[1]}x higher)")

    # 2. Optuna tuning on VALIDATION set
    print(f"\n  ðŸ” Optuna Tuning (50 trials on VALIDATION set)")
    print("  " + "-" * 56)

    mlflow.set_experiment("FraudX-Models-Fixed")

    with mlflow.start_run(run_name="LightGBM_NoLeakage"):
        
        t0 = time.time()

        study = optuna.create_study(direction="maximize")
        study.optimize(
            make_objective(X_train, y_train, X_val, y_val, class_weight),
            n_trials=50,
        ...
        print(f"\n  âœ… Best F1 on validation: {study.best_value:.4f}")
        ...
        # 5. Plot confusion matrix + curves (matches XGBoost output)
        ...
        axes[1,1].barh(range(len(indices)), importance[indices], color='darkorange')
        ...
        plt.tight_layout()
        metrics_plot = os.path.join(PLOTS_DIR, 'lightgbm_evaluation.png')
        plt.savefig(metrics_plot, bbox_inches='tight', dpi=150)
        plt.close()

        # 6. SHAP
        print("\n  ðŸ” Computing SHAP values (500 samples)...")
        ...
        shap_plot = os.path.join(PLOTS_DIR, 'lightgbm_shap_summary.png')
        plt.savefig(shap_plot, bbox_inches='tight', dpi=150)
        plt.close()

        # 7. MLflow logging
        # Convert class_weight to MLflow-safe format (string keys)
        mlflow_safe_params = {k: v for k, v in best_params.items() if k != "class_weight"}
        mlflow_safe_params["class_weight_0"] = class_weight[0]
        mlflow_safe_params["class_weight_1"] = class_weight[1]
        
        mlflow.log_params(mlflow_safe_params)
        ...
        # Make best_params JSON-safe (class_weight has int keys → convert to str keys)
        json_safe_params = {}
        for k, v in best_params.items():
            if k == "class_weight":
                json_safe_params[k] = {"0": class_weight[0], "1": class_weight[1]}
            else:
                json_safe_params[k] = v
        ...
        print(f"\n  âœ… Model saved: {os.path.join(MODELS_DIR, 'lightgbm_model.pkl')}")
        print(f"  âœ… Metrics saved: {os.path.join(MODELS_DIR, 'lightgbm_metrics.json')}")
